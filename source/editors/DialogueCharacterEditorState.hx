...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	\nRキーでカメラリセット
	\nHキーでゴースト
	\n[赤色] W: 上に移動 / S: 下に移動 / A: 左に移動 / D: 右に移動
	\n[青色] ↑: 上に移動 / ↓: 下に移動 / ←: 左に移動 / →: 右に移動
	...
	override function create() {
		...
		daText = new TypedAlphabet(DialogueBoxPsych.DEFAULT_TEXT_X, DialogueBoxPsych.DEFAULT_TEXT_Y, '', 0.05, false, 24);
	...
	function addEditorBox() {
		var tabs = [
			{name: 'Character Type', label: 'キャラクターの種類'},
		];
	...
	function addTypeUI() {
		...
		centerCheckbox = new FlxUICheckBox(leftCheckbox.x, leftCheckbox.y + 40, null, null, "真ん中", 100);
	...
	function addAnimationsUI() {
		...
		var addUpdateButton:FlxButton = new FlxButton(10, idleInputText.y + 30, "追加して更新", function() {
		...
		tab_group.add(new FlxText(idleInputText.x, idleInputText.y - 18, 0, 'アイドルと終了時の名前(.XMLファイル)'));
	...
	function addCharacterUI() {
		...
		tab_group.add(new FlxText(10, xStepper.y - 18, 0, '位置(X / Y)'));
		...
		var saveButton:FlxButton = new FlxButton(loadButton.x, reloadImageButton.y - 25, "キャラクター保存", function() {
			saveCharacter();
		});
