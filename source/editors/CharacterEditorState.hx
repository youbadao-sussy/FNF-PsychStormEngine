...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var tipTextArray:Array<String> = "[ズーム] Q: 縮小 / E: 拡大
		\nRキーでカメラリセット
		\n'[カメラ移動] J: 左に移動 / L: 右に移動 / K: 下に移動 / I: 上に移動
		\nW/Sでアニメーションタイプ変更
		\nSpaceでアニメーションを再生
		\n[キャラクター位置変更] ↑: 上に移動 / ↓: 下に移動 / ←: 左に移動 / →: 右に移動
		\nTキーで位置リセット
		\nShiftを押しながら操作すると10ずつ動きます。\n".split('\n');
	...
	function addSettingsUI() {
		...
		var check_player = new FlxUICheckBox(10, 60, null, null, "プレイヤーキャラクター", 100);
		...
		var reloadCharacter:FlxButton = new FlxButton(140, 20, "再読み込み", function()
		{
			loadChar(!check_player.checked);
			reloadCharacterDropDown();
		});

		var templateCharacter:FlxButton = new FlxButton(140, 50, "テンプレートロード", function()
		{
		...
		tab_group.add(new FlxText(charDropDown.x, charDropDown.y - 18, 0, 'キャラクター',10));
	...
	function addCharacterUI() {
		...
		var reloadImage:FlxButton = new FlxButton(imageInputText.x + 210, imageInputText.y - 3, "再読み込み", function()
		{
		...
		var decideIconColor:FlxButton = new FlxButton(reloadImage.x, reloadImage.y + 30, "アイコンカラー取得", function()
			{
		...
		flipXCheckBox = new FlxUICheckBox(singDurationStepper.x + 80, singDurationStepper.y, null, null, "フリップX", 50);
		...
		noAntialiasingCheckBox = new FlxUICheckBox(flipXCheckBox.x, flipXCheckBox.y + 40, null, null, "アンチエイリアシングなし", 80);
		...
		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x, noAntialiasingCheckBox.y + 40, "キャラクター保存", function() {
			saveCharacter();
		});
		...
		tab_group.add(new FlxText(positionCameraXStepper.x, positionCameraXStepper.y - 18, 0, 'カメラ位置(X / Y)',10));
		tab_group.add(new FlxText(healthColorStepperR.x, healthColorStepperR.y - 18, 0, '体力バーの色（赤/緑/青）',10));
	...
	function addAnimationsUI() {
		...
		tab_group.add(new FlxText(animationDropDown.x, animationDropDown.y - 18, 0, 'アニメーション',10));
		tab_group.add(new FlxText(ghostDropDown.x, ghostDropDown.y - 18, 0, 'アニメーションゴースト',10));
		tab_group.add(new FlxText(animationInputText.x, animationInputText.y - 18, 0, 'アニメーションの名前',10));
		tab_group.add(new FlxText(animationNameFramerate.x, animationNameFramerate.y - 18, 0, 'フレームレート(Fps)',10));
		tab_group.add(new FlxText(animationNameInputText.x, animationNameInputText.y - 18, 0, '.XML/.TXTファイル上のアニメーション',10));
		tab_group.add(new FlxText(animationIndicesInputText.x, animationIndicesInputText.y - 18, 0, 'アドバンス・アニメーション指標',10));
	...
	function reloadCharacterImage() {
		...
		if(Paths.fileExists('images/' + char.imageFile + '/Animation.json', TEXT)) {
			...
			char.frames = Paths.getMultiAtlas(char.imageFile);
		}
	...
	function genBoyOffsets():Void
	{
		...
		if(dumbTexts.length < 1) {
			var text:FlxText = new FlxText(10, 38, 0, "アニメーションが見つかりませんでした！", 15);
			text.scrollFactor.set();
			text.borderSize = 1;
			dumbTexts.add(text);
	...
	function updatePresence() {
		...
		DiscordClient.changePresence("キャラクター編集", "Character: " + daAnim, leHealthIcon.getCharacter());
		#end
	}
