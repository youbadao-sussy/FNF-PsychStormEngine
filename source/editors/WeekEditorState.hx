...
class WeekEditorState extends MusicBeatState
{
	...
	function addEditorBox() {
		...
		var loadWeekButton:FlxButton = new FlxButton(0, 650, "Weekロード", function() {
			loadWeek();
		});
		...
		var freeplayButton:FlxButton = new FlxButton(0, 650, "フリープレイ", function() {
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 650, "Week保存", function() {
			saveWeek(weekFile);
		});
	...
	function addWeekUI() {
		...
		hideCheckbox = new FlxUICheckBox(10, weekFileInputText.y + 40, null, null, "ストーリーモードでは非表示にする", 100);
		...
		tab_group.add(new FlxText(opponentInputText.x, opponentInputText.y - 18, 0, 'キャラクター'));
		tab_group.add(new FlxText(backgroundInputText.x, backgroundInputText.y - 18, 0, '背景アセット'));
		tab_group.add(new FlxText(displayNameInputText.x, displayNameInputText.y - 18, 0, '表示名(Weekリセット時)'));
		tab_group.add(new FlxText(weekNameInputText.x, weekNameInputText.y - 18, 0, 'Weekの名前'));
		tab_group.add(new FlxText(weekFileInputText.x, weekFileInputText.y - 18, 0, 'Week画像の名前'));
	...
	function addOtherUI() {
		...
		lockedCheckbox = new FlxUICheckBox(10, 30, null, null, "最初はウィークをロックする", 100);
		...
		hiddenUntilUnlockCheckbox = new FlxUICheckBox(10, lockedCheckbox.y + 25, null, null, "ロック解除まで非表示", 110);
		...
		tab_group.add(new FlxText(weekBeforeInputText.x, weekBeforeInputText.y - 28, 0, 'ロック解除のためにクリアしないといけないWeek'));
		tab_group.add(new FlxText(difficultiesInputText.x, difficultiesInputText.y - 20, 0, '難易度'));
		tab_group.add(new FlxText(difficultiesInputText.x, difficultiesInputText.y + 20, 0, 'デフォルト難易度は"Easy, Normal, Hard "です。'));
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	function addEditorBox() {
		...
		var loadWeekButton:FlxButton = new FlxButton(0, 685, "Weekロード", function() {
		...
		var storyModeButton:FlxButton = new FlxButton(0, 685, "ストーリーモード", function() {
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 685, "Week保存", function() {
	...
	function addFreeplayUI() {
		...
		var hideFreeplayCheckbox:FlxUICheckBox = new FlxUICheckBox(10, iconInputText.y + 30, null, null, "フリープレイでは非表示にする", 100);
		...
		tab_group.add(new FlxText(10, bgColorStepperR.y - 18, 0, '背景の色 赤/緑/青:'));
		tab_group.add(new FlxText(10, iconInputText.y - 18, 0, 'アイコン設定'));
