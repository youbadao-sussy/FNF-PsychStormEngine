...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		daText = new TypedAlphabet(DialogueBoxPsych.DEFAULT_TEXT_X, DialogueBoxPsych.DEFAULT_TEXT_Y, DEFAULT_TEXT, DEFAULT_FONT, 24);
		add(daText);
		changeText();
		super.create();
	}
	...
	function addEditorBox() {
		var tabs = [
			{name: 'Dialogue Line', label: 'ダイアログ設定'},
		];
	...
	function addDialogueLineUI() {
		...
		angryCheckbox = new FlxUICheckBox(speedStepper.x + 120, speedStepper.y, null, null, "怒りテキストボックス", 200);
		...
		fontInputText = new FlxUIInputText(10, lineInputText.y + 35, 80, DialogueCharacter.DEFAULT_CHARACTER, 8);
		blockPressWhileTypingOn.push(fontInputText);

		var loadButton:FlxButton = new FlxButton(20, fontInputText.y + 25, "ダイアログロード", function() {
			loadDialogue();
		});
		var saveButton:FlxButton = new FlxButton(loadButton.x + 120, loadButton.y, "ダイアログ保存", function() {
			saveDialogue();
		});

		tab_group.add(new FlxText(10, speedStepper.y - 18, 0, 'インターバル/スピード（ミリ秒）'));
		tab_group.add(new FlxText(10, characterInputText.y - 18, 0, 'キャラクター'));
		tab_group.add(new FlxText(10, soundInputText.y - 18, 0, 'サウンドファイル名'));
		tab_group.add(new FlxText(10, soundInputText.y - 18, 0, 'サウンドファイル名'));
		tab_group.add(new FlxText(10, lineInputText.y - 18, 0, 'テキスト'));
		tab_group.add(new FlxText(10, fontInputText.y - 18, 0, 'フォント'));
	...
	function reloadCharacter() {
		...
		if(character.animation.curAnim != null && character.jsonFile.animations != null) {
			animText.text = 'アニメーション: ' + character.jsonFile.animations[curAnim].anim + '  ' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + '  - W/Sでアニメーションタイプ変更';
		} else {
			animText.text = 'アニメーションが見つかりません！';
		}
	}

	private static var DEFAULT_TEXT:String = "coolswag";
	private static var DEFAULT_FONT:String = "dialogue.ttf";
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if (sender == fontInputText)
			{
				...
				dialogueFile.dialogue[curSelected].font = fontInputText.text;
				reloadText(true);
			}
		...
		if(!blockInput) {
			...
			for (i in 0...controlAnim.length) {
				if(controlAnim[i] && character.jsonFile.animations.length > 0) {
					...
					animText.text = 'アニメーション: ' + animToPlay + '  ' + (curAnim + 1) +' / ' + character.jsonFile.animations.length + '  - W/Sでアニメーションタイプ変更';
				}
				if(controlText[i]) {
	...
	function changeText(add:Int = 0) {
		...
		if (curDialogue.font == null) fontInputText.text = '';
		else fontInputText.text = curDialogue.font;
		...
		if(leLength > 0) {
			...
			character.playAnim(character.jsonFile.animations[curAnim].anim, daText.finishedText);
			animText.text = 'アニメーション: ' + character.jsonFile.animations[curAnim].anim + ' (' + (curAnim + 1) +' / ' + leLength + ') - W/Sでアニメーションタイプ変更';
		} else {
			animText.text = 'アニメーションが見つかりません！';
		}
		characterAnimSpeed();

		selectedText.text = 'ダイアログ:' + (curSelected + 1) + ' / ' + dialogueFile.dialogue.length + ') - A: 戻る / D: 進む';
	}
