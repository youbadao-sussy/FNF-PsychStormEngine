...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Hey!', "Plays the \"Hey!\" animation from Bopeebo,\nValue 1: BF = Only Boyfriend, GF = Only Girlfriend, \nDad = Only Dad, Something else = Both.\nValue 2: Custom animation duration,\nleave it blank for 0.6s"],
	...
	**/
	...
	override function create()
	{
		...
		text =
		"WまたはSキー / マウスホイール - 打ち込み位置変更
		\nA/D - 前や後ろのセクションへの移動
		\n左矢印または右矢印キー - スナップの変更
		\n上矢印または下矢印キー - 1ステップ移動
		\nShiftキーを押しながらセクション移動すると、一気に4セクション移動できます。
		\nコントロールキー(Ctrl)を押しながらノーツまたはイベントを押すと、編集ができます。
		\nZ: 縮小 / X: 拡大
		\nEscキー - チャートの確認(ノーツだけ再生されます)
		\nEnterキー - チャートの再生(プレイ画面に戻ります)
		\nQキーまたはEキー(ノーツ選択時) - ノーツの伸ばす量を変更
		\nSpaceキー - 曲の再生/停止";
	...
	function addSectionUI():Void
	{
		...
		tab_group_section.add(new FlxText(stepperBeats.x, stepperBeats.y - 15, 0, 'セクションあたりの拍数'));
	...
	function addNoteUI():Void
	{
		...
		tab_group_note.add(new FlxText(10, 50, 0, '伸ばす時間（ミリ秒）'));
		tab_group_note.add(new FlxText(10, 90, 0, 'ノーツの種類'));
	...
	function addEventsUI():Void
	{
		...
		selectedEventText = new FlxText(addButton.x - 100, addButton.y + addButton.height + 6, (moveRightButton.x - addButton.x) + 186, '選択されているイベント: なし');
	...
	function changeEventSelected(change:Int = 0)
	{
		if(curSelectedNote != null && curSelectedNote[2] == null) //Is event note
		{
			...
			selectedEventText.text = '選択されているイベント: ' + (curEventSelected + 1) + ' / ' + curSelectedNote[1].length;
		}
		else
		{
			curEventSelected = 0;
			selectedEventText.text = '選択されているイベント: なし';
		}
		updateNoteUI();
	}
	...
	function addChartingUI() {
		...
		tab_group_chart.add(new FlxText(metronomeOffsetStepper.x, metronomeOffsetStepper.y - 15, 0, 'オフセット (ms):'));
		tab_group_chart.add(new FlxText(instVolume.x, instVolume.y - 15, 0, 'インストの音量'));
		tab_group_chart.add(new FlxText(voicesVolume.x, voicesVolume.y - 15, 0, 'ボイスの音量'));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				case 'ノーツを打つプレイヤーをチェンジする'://たぶんできなかった原因　ココ
