...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		DiscordClient.changePresence("メニューキャラクター編集", "Editting: " + characterFile.image);
		#end
		...
		var tipText:FlxText = new FlxText(0, 540, FlxG.width,
			"矢印キー - オフセットを変える （Shift + 矢印キーを押すと10移動します）
			\nSpace - 再生 \"スタートプレス\" アニメーション （プレイヤー キャラクター タイプ）", 16);
	...
	function addEditorBox() {
		...
		var saveButton:FlxButton = new FlxButton(0, 480, "キャラクター保存", function() {
			saveCharacter();
		});
	...
	function addTypeUI() {
		...
		opponentCheckbox = new FlxUICheckBox(10, 20, null, null, "相手", 100);
		...
		boyfriendCheckbox = new FlxUICheckBox(opponentCheckbox.x, opponentCheckbox.y + 40, null, null, "プレイヤー", 100);
		...
		girlfriendCheckbox = new FlxUICheckBox(boyfriendCheckbox.x, boyfriendCheckbox.y + 40, null, null, "ガールフレンド", 100);
	...
	function addCharacterUI() {
		...
		flipXCheckbox = new FlxUICheckBox(10, confirmInputText.y + 30, null, null, "フリップ X軸", 100);
		...
		confirmDescText = new FlxText(10, confirmInputText.y - 18, 0, '再生するアニメーションのxml', 10);
		tab_group.add(new FlxText(10, imageInputText.y - 18, 0, 'イメージファイル名', 10));
		tab_group.add(new FlxText(10, idleInputText.y - 18, 0, 'アイドル時のアニメーションのxml', 10));
		tab_group.add(new FlxText(scaleStepper.x, scaleStepper.y - 18, 0, '大きさ', 10));
