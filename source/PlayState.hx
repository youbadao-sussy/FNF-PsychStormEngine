...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['F', 0.1], //From 0% to 9%
		['E', 0.2], //From 10% to 19%
		['D-', 0.3], //From 20% to 29.99%
		['D', 0.4], //From 30% to 39.99%
		['D+', 0.5], //From 40% to 49.99%
		['C-', 0.67], //From 50% to 56.99%
		['C', 0.63], //From 57% to 62.99%
		['C+', 0.69], //From 63% to 68.99%
		['B-', 0.76], //From 69% to 85.99%
		['B', 0.83], //From 76% to 82.99%
		['B+', 0.89], //From 83% to 88.99%
		['A-', 0.92], //From 89% to 91.99%
		['A', 0.94], //From 92% to 93.99%
		['A+', 0.975], //From 94% to 96.99%
		['S', 0.99], //From 97.5% to 99%
		['SS', 1], //From 98.43% to 98.87%
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("GameF.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("GameF.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("GameF.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		if(instakillOnMiss == false) {
			if(ratingName == '?') {
				scoreTxt.text = 'スコア: ' + songScore + ' | ミス数: ' + songMisses + ' | レーティング: ' + ratingName;
			} else {
				scoreTxt.text = 'スコア: ' + songScore + ' | ミス数: ' + songMisses + ' | レーティング: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
			}if(botplayTxt.visible) {
				scoreTxt.text = 'オートプレイ';
			}
		}
		if(instakillOnMiss == true) {
			if(ratingName == '?') {
				...
				scoreTxt.text = 'スコア: ' + songScore + ' - (' + ratingFC + ') ' + ratingName;//peeps wanted no integer rating
			}if(botplayTxt.visible) {
				scoreTxt.text = 'オートプレイ';
			}
		}
