...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "Modパックが見つかりません\n一度終了してからModパックを入れてください", 48);
		...
		startX -= 190;
		buttonDisableAll = new FlxButton(startX, 0, "すべて無効", function() {
			for (i in modsList)
			{
				i[1] = false;
			}
		...
		while (i < modsList.length)
		{
			...
			newMod.alphabet.size = Std.int(42 * scale);
		...
		for (mod in mods)
		{
			...
			if(i == curSelected)
			{
				...
				if (mod.restart){//finna make it to where if nothing changed then it won't reset
					descriptionTxt.text += "ゲームを再起動してください！";
				}
